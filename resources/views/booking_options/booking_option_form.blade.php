...
@section('breadcrumbs')
    ...
    @isset($bookingOption)
        <x-nav.breadcrumb href="{{ route('booking-options.show', [$service, $bookingOption]) }}">
            {{ $bookingOption->name }}
        </x-nav.breadcrumb>
    @endisset
    <x-nav.breadcrumb/>
                    ...
                    <div id="slugHint" class="form-text">
                        {!! __('This field defines the path in the URL, such as :url. If you leave it empty, is auto-generated for you.', [
                            'url' => isset($bookingOption->slug)
                                ? sprintf('<a href="%s" target="_blank">%s</a>', route('booking-options.show', [$service, $bookingOption]), route('booking-options.show', [$service, $bookingOption], false))
